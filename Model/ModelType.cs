...
namespace Model
{
    public class ModelType
    {
        ...
        public record Category(Int64 Id, Guid categoryId, string name, string image, DateTime createdDate, string status, bool active);
        public record SubCategory(Int64 id, Guid categoryId, string categoryName, Guid subCategoryId, string name, DateTime createdDate, string status, bool active);
        public record SubCategoryType(Int64 id, Guid categoryId, string categorName, Guid subCategoryId, string subCategoryName, Guid subCategoryTypeId, string name, DateTime createdDate, string status, bool active);


        public record SubCategoryTypeForUser(Int64 id, Guid categoryId, string categorName, Guid subCategoryId, string subCategoryName,
            Guid subCategoryTypeId, string name, DateTime createdDate, string status, bool active);

        public record Seller(Int64 id, Guid sellerId, string name, string mobile, string email, string streetAddress, string state, string city, string pincode,
            string country, string description, DateTime createdDate, string status, bool active);

        public record Brand(Int64 id, Guid brandId, string name, string description, DateTime createdDate, string status, bool active);
        public record Color(Int64 id, Guid colorId, string name, string code, DateTime createdDate, Guid createdBy, string status, bool active);
        public record SizeRes(Int64 id, Guid sizeId, string name, string code, DateTime createdDate, string status, bool active);

        public record Product(Int64 id, Guid productId, Guid categoryId, string categoryName, Guid subCategoryId, string subCategoryName,
    ...
    decimal DiscountPrice, decimal TotalAmount, string Username, string Name, string Image, string ProductName);
       ...
       string trackingNo, string note, string status, DateTime createdDate);
        ...
        public record Shipping(Int64 id, Guid shippingId, Guid orderId, string trackingNo, string carrier, DateTime estimateDelivery, string status, DateTime createdDate);

        public record Banner(Int64 id, Guid bannerId, Guid categoryId, Guid subCategoryId, Guid subCategoryTypeId, string image,
            string title, string subTitle, DateTime createdDate, string status, bool active);

        public record BannerForUser(Int64 id, Guid bannerId, Guid categoryId, Guid subCategoryId, Guid subCategoryTypeId, string image,
            string title, string subTitle, DateTime createdDate);

        public record Payment(Int64 id, Guid paymentId, Guid orderId, string paymentMethod, string transactionId, string status, DateTime createdDate);
        public record PaymentMode(Int64 id, Guid paymentModeId, string paymentName, DateTime createdDate, string status, bool active);
    ...
    string state, string city, string pincode, string country);
        ...
        //public record AllOrderByUserId(Int64 id, string shippedDate, decimal price, decimal discountPrice, decimal deliveryCharge, decimal gstCharge, decimal extraCharge,
        //    decimal totalAmount, string paymentMethod, string transactionId, string trackingNo, string note, string status, DateTime createdDate, string orderNo, string firstName,
        //    string middleName, string lastName, string email, string phoneNumber, string productName, string subName, string addressName, string addressMobile,
        //    string addressEmail, string streetAddress, string state, string city, string pincode, string country);

        public record UserProfile(Guid userId, string username, string firstName, string middleName, string lastName, string email, string phoneNumber, DateTime createdDate);

        public record Address(long id,Guid addressId,int isDefualt, string name,string mobile,string email, string streetAddress,string state,string city,string pincode,string country,DateTime createdDate,string Status,bool active ,string city_Name, string state_Name, string country_Name);

        public record CartItemCount
        {
            public Int64 ItemCount { get; init; }

            public CartItemCount() { } // Parameterless constructor
            public CartItemCount(Int64 itemCount) => ItemCount = itemCount; // Your original constructor
        }

        //public record CartItemCount(Int64 ItemCount);
        public record Cart(Guid productId, string productName, string subName, string image, decimal price, decimal discountPrice,
            Int32 quantity, decimal totalPrice);
        public record ShippingMethod(Int64 Id, Guid shippingMethodId, string name, string courier, string shippingZone, string baseCost, string costPerKg,
            string maxWeightLimit, string minOrderValue, string trackingURL, DateTime createdDate, string Status, bool active);

        public record IngredientMethod(Int64 Id, Guid ingredientId, string name, string description, DateTime createdDate, string Status, bool active);
        public record class CountryMethod(int Country_Id, string Country_Code, string Country_Name, int phonecode, bool IsActive)
        {
            public CountryMethod() : this(0, string.Empty, string.Empty, 0, false) { }
        }

        public record class StateMethod(int Pk_StateId, string StateName, int Fk_CountryId)
        {
            public StateMethod() : this(0, string.Empty, 0) { }
        }
        public record City(int Fk_StateId, string CityName, int Pk_CityId);
